...
同样的 `std::promise` **只能移动**，不可复制，所以我们使用 `std::move` 传递参数。

如你所见，它的基本使用很简单，不过我们可以举一个更加复杂的需求与示例。

---

在编写网络服务器之类的程序时，我们经常要处理多个客户端连接的情况，你可能会让每个线程处理一个连接，让网络尽早联通。但是，随着连接数量的增加，这种方式会越来越不合适。因为连接数量增加则意味着线程数量的增多，而大量的线程会消耗大量的系统资源，会造成上下文频繁切换，这都会对性能有影响。因此通常是通过少数线程处理网络连接，而**每个线程同时要处理多个连接**，对于需要处理大量网络连接的应用而言，这是一种普遍的做法。

- 简而言之，一个线程处理一个连接（客户端）通常不现实，即**一对一不可**，通常是**一对多**。

我们使用 [`Boost.Asio`](https://www.boost.org/doc/libs/1_85_0/doc/html/boost_asio.html) 网络库，写一个使用 `std::promise` 解决单线程多连接的示例：
