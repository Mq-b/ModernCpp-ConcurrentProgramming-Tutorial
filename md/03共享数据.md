...
`std::shared_mutex` 同样支持 `std::lock_guard`、`std::unique_lock`。和 `std::mutex` 做的一样，保证*写线程*的独占访问。**而那些无需修改数据结构*读线程*，可以使用 [`std::shared_lock<std::shared_mutex>`](https://zh.cppreference.com/w/cpp/thread/shared_lock) 获取访问权**。
